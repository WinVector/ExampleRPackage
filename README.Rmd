 ...
 * A working `R` of version at least as new as `3.6`, with `4.0.2` preferred.
 ...
 * For the source-control steps a `git` client (either command line or graphical). `git` can be gotten from [https://git-scm.com](https://git-scm.com).
...
We assume your machine has a current working R, command-shell (bash / zsh), text editor (emacs, vim, or other), R, C compliler, git, and Latex.
...
These steps require some knowlege of working with your computer, network access, disk space, and admin rights. We strongly advise you take the steps in this section before class.

Once you have your machine configured start up `R` and install the package we will be using.
...
One could try to issue the clone command using [git2r](https://CRAN.R-project.org/package=git2r).
